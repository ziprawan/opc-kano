...
func ExtractChunksFromWebP(webpData []byte) (WebPChunk, error) {
	webpChunk := WebPChunk{}
	...
	for {
		...
		webpChunk.SetChunk(chunkNameStr, chunkPayload)
	...
	return webpChunk, nil
}

func ExtractChunksFromWebPOld(webpData []byte) (Chunks, error) {
	// Check the first twelve bytes of webp data
	// First four byte must be "RIFF"
	// Second four byte represent the RIFF payload size
	riffHeader := webpData[:8]
